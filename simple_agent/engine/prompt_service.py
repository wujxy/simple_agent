...
class PromptService:
    def build_action_prompt(
        ...
        if session.current_plan:
            plan_summary = session.current_plan.get("summary")

        current_step = None
        if session.current_plan and session.current_plan.get("steps"):
    ...
    def build_planning_prompt(self, session: SessionState, turn: TurnState, context: dict) -> str:
        return build_planner_prompt(turn.user_message)
    ...
    def build_replanning_prompt(
        ...
        return build_replan_prompt(turn.user_message, failed_step, reason, completed_steps)

    def build_verification_prompt(self, session: SessionState, turn: TurnState, context: dict) -> str:
        actions_summary = self._format_memory(context.get("important_memory", []))
        return build_verify_prompt(turn.user_message, actions_summary)

    def build_summary_prompt(self, session: SessionState, turn: TurnState, context: dict) -> str:
        actions_summary = self._format_memory(context.get("important_memory", []))
        return build_summary_prompt(turn.user_message, actions_summary)
